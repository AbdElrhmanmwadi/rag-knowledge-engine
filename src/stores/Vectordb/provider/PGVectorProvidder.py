...
class PGVectorDBProvider(VectorDBInterface):
    ...
    async def search_by_vector(self, collection_name: str, vector: list, limit: int):
        ...
        async with self.db_client() as session:
            async with session.begin():
                search_sql = sql_text(f'SELECT {PgVectorTableSchemeEnums.TEXT.value} as text, 1 - ({PgVectorTableSchemeEnums.VECTOR.value} <=> :vector) as score, {PgVectorTableSchemeEnums.METADATA.value} as metadata '
                ...
                return [
                    RetrievedDocument(
                        text=record.text,
                        score=record.score,
                        meta_data=record.metadata if hasattr(record, "metadata") else None
                    )
